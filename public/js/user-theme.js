...
(function () {
  ...
  window.UserTheme = {
    ...
    /**
     * capturarEstadoDashboard() - Captura el estado actual del dashboard.
     * 
     * Este método lee el DOM actual para extraer información sobre:
     * - Paneles colapsados
     * - Tabs activos
     * - Nivel de zoom
     * - Otras preferencias visuales
     * 
     * Retorna un objeto JSON que se guarda en localStorage y se sincroniza
     * con BD al hacer logout.
     * 
     * NOTA: Personalizar según tu estructura de dashboard actual.
     */
    capturarEstadoDashboard() {
      const estado = {
        collapsedPanels: [],
        activeTab: null,
        zoom: 100,
      ...
      // ── Buscar paneles colapsados (si existen elementos con clase 'collapsed')
      const paneles = document.querySelectorAll('[data-panel]');
      paneles.forEach(panel => {
        if (panel.classList.contains('collapsed')) {
          estado.collapsedPanels.push(panel.getAttribute('data-panel'));
        }
      });
      
      // ── Buscar tab activo (si existen tabs con clase 'active')
      const tabActivo = document.querySelector('[role="tab"].active') || document.querySelector('.nav-link.active');
      if (tabActivo) {
        estado.activeTab = tabActivo.getAttribute('id') || tabActivo.textContent;
      }
      
      // ── Guardar en localStorage
      localStorage.setItem('dashboardState', JSON.stringify(estado));
      
      return estado;
    },
    
    /**
     * restaurarEstadoDashboard() - Restaura el estado del dashboard desde localStorage/BD.
     * 
     * Se ejecuta después del login para restaurar las preferencias del usuario.
     * Lee 'dashboardState' de localStorage y aplica los cambios al DOM.
     */
    restaurarEstadoDashboard() {
      try {
        const estadoJson = localStorage.getItem('dashboardState');
        if (!estadoJson) return;
        
        const estado = JSON.parse(estadoJson);
        
        // ── Restaurar paneles colapsados
        if (estado.collapsedPanels && Array.isArray(estado.collapsedPanels)) {
          estado.collapsedPanels.forEach(panelId => {
        ...
        if (estado.activeTab) {
          const tab = document.getElementById(estado.activeTab) || 
                      document.querySelector(`[data-tab="${estado.activeTab}"]`);
          if (tab) {
            tab.classList.add('active');
            // Cambiar contenido si es necesario
            const tabContent = document.querySelector(tab.getAttribute('data-target'));
            if (tabContent) {
        ...
        console.warn("Error restaurando estado del dashboard:", e);
      }
    }
  };
